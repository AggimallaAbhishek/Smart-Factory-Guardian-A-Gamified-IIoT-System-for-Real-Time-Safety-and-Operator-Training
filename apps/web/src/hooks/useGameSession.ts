...
import type { AlertEventPayload, AlertType, PlayerSession } from "@guardian/protocol";
import { DEFAULT_SESSION_DURATION_SEC } from "../lib/constants";

export function useGameSession(playerName: string) {
  const [state, setState] = useState<GameState>(createInitialGameState(DEFAULT_SESSION_DURATION_SEC));
  ...
  const ingestAlert = useCallback((payload: AlertEventPayload) => {
    console.debug("[game] alert ingested", payload);
    setState((previous) =>
      connectAlert(previous, {
  ...
  const respond = useCallback((alertType: AlertType) => {
  ...
  const buildSessionSummary = useCallback((): PlayerSession => {
